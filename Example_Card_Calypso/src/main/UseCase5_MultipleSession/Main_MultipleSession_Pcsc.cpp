...
/**
 * Handles the process of executing multiple sessions on a Calypso card using
 * the PC/SC plugin, particularly to bypass the card's modification buffer
 * limitation.
 *
 * <p>This class illustrates a technique to overcome the restriction imposed
 * by the modification buffer size of a Calypso card by utilizing multiple
 * session mode.
 */
...
runExample() {
    ...
    /*
     * Compute the number of append records (29 bytes) commands that will
     * overflow the card modifications buffer. Each append records will
     * consume 35 (29 + 6) bytes in the buffer.
     *
    ...
    logger->info(
        "==== Send % Append Record commands. Modifications buffer capacity "
        "= % bytes i.e. % 29-byte commands ====\n",
        nbCommands,
        modificationsBufferSize,
        modificationsBufferSize / 35);

    const std::vector<std::uint8_t> eventLogDataFill(
        HexUtil::toByteArray(EVENT_LOG_DATA_FILL));

    for (int i = 0; i < nbCommands; i++) {
        cardTransaction->prepareAppendRecord(SFI_EVENT_LOG, eventLogDataFill);
    }

    cardTransaction->prepareCloseSecureSession().processCommands(
