...
/**
 * Handles the process of debiting the Stored Value (SV) of a Calypso card
 * using the PC/SC plugin and the Calypso Card Extension Service.
 *
 * <p>This class demonstrates the SV debit process inside a Secure Session.
 */
...
runExample() {
    logger->info(
        "= UseCase Calypso #8: Stored Value debit ==================\n");
     ...
     * itself expose prepareOpenSecureSession()/prepareSvGet() (they're only
     * declared on SecureSymmetricCryptoTransactionManager<T>, a sibling
     * interface implemented by the same concrete object). A downcast is
    ...
    cardTransaction->prepareSvDebit(2)
